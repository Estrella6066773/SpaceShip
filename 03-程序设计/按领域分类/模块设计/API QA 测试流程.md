...
> 表内「操作」列给出**真实英文方法签名**（`方法名(参数)`），图内节点与 C# 直接调用均可用同一签名——适配层方法名与核心层一致（枚举参数核心层用类型化值，适配层用字符串）。参数值为**英文枚举名**（如 `"Core"`、`"Forward"`、`"Food"`），`ApiEnum` 自动解析（也接受中文标签）；商品名等数据字符串按实际配置填写。核心层类型化签名对应关系见《API 框架.md》第 5 节。
